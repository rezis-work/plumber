...
/// Hard filters per implementation_003 §5 (strict): area-aware `plumber_service_areas`, distance within `service_radius_km`.
pub async fn fetch_candidates_strict<'c, E>(
    executor: E,
...
/// City-wide fallback (Implementation 004 §4.3): same service/emergency/dispatch exclusions; city tie via
/// `plumber_service_areas.city_id` or `plumber_profiles.current_city_id`; **no** radius filter (distance still for ranking).
pub async fn fetch_candidates_city_fallback<'c, E>(
    executor: E,
    input: &MatcherOrderInput,
    config: &MatcherConfig,
) -> Result<Vec<MatcherCandidate>, sqlx::Error>
        ...
        FROM (
            ...
            WHERE pp.is_approved = true
              ...
              AND (
                  ...
                  OR pp.current_city_id IS NOT DISTINCT FROM $2
              )
        ) sub
        "#,
    )
